...
function injectShellScript(response){
  ...
  return response.text().then(html=>{
    ...
    html=html.replace(/shell-location-status\.js\?v=[^"'<>]+/g,'shell-location-status.js?v=0.31.10-test.49');
    let injection='';
    ...
    if(!html.includes('shell-location-status.js'))injection+=`<script src="./shell-location-status.js?v=0.31.10-test.49"></script>`;

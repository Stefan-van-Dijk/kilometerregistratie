(function () {
  ...
  function selectSection(next) {
    ...
    else ensureOriginalMode('kilometers');
    closeDrawer();
    showSection();
  }
      ...
      <div class="km-shell-location-node" data-shell-location-node="${esc(location.id)}" data-depth="${depth}" style="--depth:${depth}">
        <div class="km-shell-location-row" data-shell-location-toggle="${esc(location.id)}">
          ...
          <div class="km-shell-location-buttons">
            ...
            <button type="button" aria-hidden="true">${expanded ? '⌄' : '›'}</button>
          </div>
        </div>
  ...
  function openSettingsSheet() {
    ...
    if (section === 'time') mountTimeSettings();
    else mountKmSettings();
  ...
  function restoreKmApp() {
    ...
    const topAction = $('#topAction');
    if (topAction?.dataset.action === 'home') topAction.click();
  }
  ...
  function closeSettingsSheet() {
    ...
    if (section === 'time') restoreTimeFrame();
    else restoreKmApp();
  ...
  function bindGlobalEvents() {
    ...
    window.addEventListener('storage', event => {
      if ([DATA_KEY, 'urenregistratie.pwa.v1'].includes(event.key)) {
        renderLocations();
        syncChrome();
      }
    });
